...
def bank_clustering():
    ...
    kmeans_labels_original = kmeans_original.fit_predict(X_preprocessed)

    # EM Clustering in original space
    em_original = GaussianMixture(n_components=2, random_state=42)
    em_labels_original = em_original.fit_predict(X_preprocessed)

    # Dimensionality Reduction
    pca_components = 9
    X_pca, explained_variance = evaluate_pca(X_preprocessed, pca_components)

    ica_components = 9
    X_ica, kurt = evaluate_ica(X_preprocessed, ica_components)

    rp_components = 9
    X_rp, reconstruction_error = evaluate_rp(X_preprocessed, rp_components)
